...
def run_tree():
    ...
    # Create a logger
    log = Log(args.log_dir+'/'+args.attribute)
    print("Log dir: ", args.log_dir, flush=True)
    # Create a csv log for storing the test accuracy, mean train accuracy and mean loss for each epoch
    log.create_log('log_epoch_overview', 'epoch', 'test_acc','test_auc', 'mean_train_acc','mean_train_auc', 'mean_train_crossentropy_loss_during_epoch')
        ...
        for epoch in range(epoch, args.epochs + 1):
            ...
            if args.epochs>100:
                if epoch%10==0 or epoch==args.epochs:
                    ...
                    log.log_values('log_epoch_overview', epoch, "n.a.", "n.a", train_info['train_accuracy'], train_info['train_auc'], train_info['loss'])
            else:
                eval_info = eval(tree, testloader, epoch, device, log)
                original_test_acc = eval_info['test_accuracy']
                best_test_acc = save_best_test_tree(tree, optimizer, scheduler, best_test_acc, eval_info['test_accuracy'], log)
                log.log_values('log_epoch_overview', epoch, eval_info['test_accuracy'], eval_info['test_auc'], train_info['train_accuracy'], train_info['train_auc'], train_info['loss'])
