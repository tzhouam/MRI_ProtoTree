...
def run_ensemble():
    ...
    if not all_args.disable_cuda and torch.cuda.is_available():
        device = torch.device('cuda:{}'.format(all_args.cuda))
    else:
    ...
    for pt in range(1,all_args.nr_trees_ensemble+1):
        ...
        trained_tree, pruned_tree, pruned_projected_tree, original_test_acc, pruned_test_acc, pruned_projected_test_acc, project_info, eval_info_samplemax, eval_info_greedy, info_fidelity = run_tree()
